...
// ------------------------------------------
//FOR DEBUG
//imagettftext($im, 10, 0, 10, 10, $black, ARIAL_TTF, $longitude[1]);
//imagettftext($im, 10, 0, 10, 25, $black, ARIAL_TTF, $hc[2]);
...
// draw the lines for the house cusps
  ...
  for ($i = 1; $i <= 12; $i = $i + 1)
  {
    ...
    imagettftext($im, 10, 0, $xy[0] + $center_pt, $xy[1] + $center_pt, $black, FONTPATH.ARIAL_TTF, $i);
  }
...
Function drawboldtext($image, $size, $angle, $x_cord, $y_cord, $clr_to_use, $fontfile, $text, $boldness)
{
  ...
  {
    ImageTTFText($image, $size, $angle, $x_cord+$_x[$n], $y_cord+$_y[$n], $clr_to_use, FONTPATH.$fontfile, $text);
  }
}
