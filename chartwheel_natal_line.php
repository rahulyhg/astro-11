...
header("Content-type: image/png");  //тут
...
// put the signs across the top and bottom of the horizontal lines
  for ($i = 1; $i <= 12; $i++)
  {
    ...
    ImageTTFText($im, 16, 0, $x1 + $x_offset, $y_line_sign_dist1, $clr_to_use, HAMBURG_TTF, chr($sign_glyph[$i]));
    ImageTTFText($im, 16, 0, $x1 + $x_offset, $y_line_sign_dist2, $clr_to_use, HAMBURG_TTF, chr($sign_glyph[$i]));
  }
...
// create colored rectangles for each house
  for ($i = 1; $i <= 12; $i++)
  {
    ...
    $array = imagettfbbox(10, 0, ARIAL_TTF, $i);
    ...
    if (abs($hc1[$i] - $hc1[$i + 1]) < 180)
    {
      ...
      imagettftext($im, 10, 0, (($x1 + $x2) / 2) + $x_offset - ($width_of_text / 2), $y_line_dist1 + 30, $black, ARIAL_TTF, $i);
      imagettftext($im, 10, 0, (($x1 + $x2) / 2) + $x_offset - ($width_of_text / 2), $y_line_dist2 - 20, $black, ARIAL_TTF, $i);
    }
    else
    {
...
// put planets in chartwheel
  ...
  for ($i = $num_planets - 1; $i >= 0; $i--)
  {
    ...
    if ($flag == False)
    {
      imagettftext($im, 16, 0, $x1 + $x_offset, $y_line_dist1 - 20, $planet_color, HAMBURG_TTF, chr($pl_glyph[$sort_pos[$i]]));
      imagettftext($im, 16, 0, $x1 + $x_offset, $y_line_dist2 + 37, $planet_color, HAMBURG_TTF, chr($pl_glyph[$sort_pos[$i]]));
    }
    else
    {
      imagettftext($im, 16, 0, $x1 + $x_offset, $y_line_dist1 - 38, $planet_color, HAMBURG_TTF, chr($pl_glyph[$sort_pos[$i]]));
      imagettftext($im, 16, 0, $x1 + $x_offset, $y_line_dist2 + 55, $planet_color, HAMBURG_TTF, chr($pl_glyph[$sort_pos[$i]]));
    }
