...
// draw in the planet glyphs
  for ($i = 0; $i <= $num_to_use; $i++)
  {
    ...
    {
      drawboldtext($im, 16, 0, $margins + $left_margin_planet_table, $cell_height + $cell_height * ($i + 1), $blue, HAMBURG_TTF, chr($pl_glyph[$i]), 0);
    }
    else
    {
    ...
    if ($i <= $last_planet_num - 4)
    {
      drawboldtext($im, 14, 0, $margins + $left_margin_planet_table + $cell_width * 10, $cell_height + $cell_height * ($i + 1), $blue, HAMBURG_TTF, chr($sign_glyph[$sign_num2]), 0);
